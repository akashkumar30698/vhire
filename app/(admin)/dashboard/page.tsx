...
function DashboardPage() {
  ...
  {INTERVIEW_CATEGORY.map((category) => {
    const interviews = groupedInterviews[category.id];
    
    // Check if interviews for the category are defined and not empty
      ...
      return (
        <section key={category.id}>
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <Badge variant={category.variant}>{interviews.length}</Badge>
          </div>
    ...
    return null; // Return null if no interviews for this category
  })}
</div>
